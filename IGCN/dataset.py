...
class MovielensDataset(BasicDataset):
    def __init__(self, dataset_config):
        super(MovielensDataset, self).__init__(dataset_config)

        input_file_path = os.path.join(dataset_config['path'], 'ratings.dat')
        user_inter_sets, item_inter_sets = dict(), dict()
        with open(input_file_path, 'r') as f:
            line = f.readline().strip()
            print(1)
