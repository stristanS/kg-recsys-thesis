...
class SparseRowIndexer:
    ...
    def __getitem__(self, row_selector):
        # print('SHAPESHAPE', self.data.shape, self.indices.shape, self.indptr.shape)
        indices = np.concatenate(self.indices[row_selector])
        data = np.concatenate(self.data[row_selector])
        indptr = np.append(0, np.cumsum(self.indptr[row_selector]))
        shape = [indptr.shape[0] - 1, self.shape[1]]
        # print('data, indices, indptr', data, indices, indptr)
...
class SparseColIndexer:
    ...
    def __getitem__(self, col_selector):
        # print('SHAPESHAPE2222', self.data.shape, self.indices.shape, self.indptr.shape)
        # print('SHAPESHAPE2222!!!!!', self.data, 'idx', self.indices, 'indptr', self.indptr)
        # print('Acol', col_selector)
        ...
        # print('SHAPESHAPE33333', data.shape, 'idx', indices.shape, 'indptr', indptr.shape)
        # print('SHAPESHAPE33333!!!!', data, 'idx', indices, 'indptr', indptr)
        # print('data2, indices2, indptr2', data, indices, indptr)
        shape = [self.shape[0], indptr.shape[0] - 1]
        # print('SHAPESHAPE44444',self.shape, shape) 
...
class MyDynamicDataset(Dataset):
    def __init__(self, root, A, links, labels, h, sample_ratio, max_nodes_per_hop, 
                 u_features, v_features, class_values, max_num=None, test = False):
    ...
    def get(self, idx):
        ...
        # print('i, j, g_label', i, j, g_label)
        tmp = subgraph_extraction_labeling(
            (i, j), self.Arow, self.Acol, self.h, self.sample_ratio, self.max_nodes_per_hop, 
            self.u_features, self.v_features, self.class_values, g_label, self.test
        )
        # print('construct_pyg_graph(*tmp).y', construct_pyg_graph(*tmp).y)
        return construct_pyg_graph(*tmp), torch.tensor(i), torch.tensor(j)
...
def subgraph_extraction_labeling(ind, Arow, Acol, h=1, sample_ratio=1.0, max_nodes_per_hop=None, 
    ...
    # print('u_visited, v_visited', u_visited, v_visited)
    u_fringe, v_fringe = set([ind[0]]), set([ind[1]])
    # print('u_fringe, v_fringe', u_fringe, v_fringe)
    for dist in range(1, h+1):
        v_fringe, u_fringe = neighbors(u_fringe, Arow), neighbors(v_fringe, Acol)
        # print('u_fringe, v_fringe', len(u_fringe),len(v_fringe))
        u_fringe = u_fringe - u_visited
        v_fringe = v_fringe - v_visited
        # print('u_fringe, v_fringe', len(u_fringe), len(v_fringe))
        u_visited = u_visited.union(u_fringe)
        v_visited = v_visited.union(v_fringe)
        # print('u_visited', u_visited, v_visited)
        if sample_ratio < 1.0:
    ...
    # print('subgraph[0, 0]', subgraph[0, 0])
    subgraph[0, 0] = 0
    # print('np.where(subgraph ==0)',np.nonzero(subgraph.toarray()==0)[0].shape, subgraph.get_shape())
    # print('subgraph', subgraph)
    
    # prepare pyg graph constructor input
    # neg_idxs = np.random.choice(np.nonzero(subgraph.toarray()==0)[0].shape[0], size=4, replace=False)
    # u_neg, v_neg, r_neg = np.nonzero(subgraph.toarray()==0)[0][neg_idxs],\
    #                       np.nonzero(subgraph.toarray()==0)[1][neg_idxs], np.zeros(4)
    # print(u_neg, v_neg, r_neg)
    
    u, v, r = ssp.find(subgraph)  # r is 1, 2... (rating labels + 1)
    # print('u1', u.shape, v.shape, r.shape)
    # u = np.hstack((u, u_neg))
    # v = np.hstack((v, v_neg))
    # r = np.hstack((r, r_neg))
    # print('u', u.shape, v.shape, r.shape)
    v += len(u_nodes)
    if test == False:
        # print('test', r)
    ...
    # print('FINAL', u, v, r, node_labels, max_node_label, y)
    # print('LABELS', r)
