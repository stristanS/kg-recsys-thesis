...
def neg_sampling(ratings_df, n_neg=1, neg_val=0, pos_val=1, percent_print=5):
  ...
  """
  ratings_df.u_nodes = ratings_df.u_nodes.astype('category').cat.codes.values
  ratings_df.v_nodes = ratings_df.v_nodes.astype('category').cat.codes.values
  sparse_mat = coo_matrix((ratings_df.ratings, (ratings_df.u_nodes, ratings_df.v_nodes)))
  dense_mat = np.asarray(sparse_mat.todense())
  print(dense_mat.shape)
  nsamples = ratings_df[['u_nodes', 'v_nodes']]
  nsamples['ratings'] = nsamples.apply(lambda row: 1, axis=1)
  ...
  for row in dense_mat:
    ...
    i+=1
  nsamples=nsamples.append(pd.DataFrame(nTempData, columns=["u_nodes","v_nodes", "ratings"]),ignore_index=True)
  return nsamples
...
def load_data_monti(dataset, testing=False, rating_map=None, post_rating_map=None):
    ...
    if post_rating_map is None:
        rating_mx_train[train_idx] = labels[train_idx].astype(np.float32) 
    else:
        rating_mx_train[train_idx] = np.array([post_rating_map[r] for r in class_values[labels[train_idx]]]) 
...
def load_official_trainvaltest_split(dataset, testing=False, rating_map=None, post_rating_map=None, ratio=1.0):
    ...
    data_train_wo_ns = pd.read_csv(
        'raw_data/ml1m/df_train.csv', header=None,
        names=['u_nodes', 'v_nodes', 'ratings', 'timestamp'])

    "EXPERIMENT BEGIN"
    data_test_wo_ns = pd.read_csv(
        'raw_data/ml1m/df_test.csv', header=None,
        names=['u_nodes', 'v_nodes', 'ratings', 'timestamp'])
    
    data_val_wo_ns = pd.read_csv(
        'raw_data/ml1m/df_val.csv', header=None,
        names=['u_nodes', 'v_nodes', 'ratings', 'timestamp'])
    
    data_test= itertools.product(data_test_wo_ns['u_nodes'].unique(), data_test_wo_ns['v_nodes'].unique())
    data_test = pd.DataFrame(data_test, columns=["u_nodes", "v_nodes"])
    data_test['ratings'] = 0
    data_test['timestamp'] = 0
    data_test = data_test.append(data_test_wo_ns)
    data_test.sort_values(by = 'ratings', inplace = True)
    data_test.reset_index(inplace = True, drop = True)
    data_test = data_test.drop_duplicates(subset= ['u_nodes', 'v_nodes'], keep= 'last')


    "EXPERIMENT END"
    # print('data_train.shape11',data_train_wo_ns.shape)
    data_train_wo_ns = data_train_wo_ns.append(data_val_wo_ns)
    data_train = neg_sampling(data_train_wo_ns)
    # print('data_train.shape2',data_train.shape)
    data_array_train = data_train.values.tolist()
    data_array_train = np.array(data_array_train)
    data_array_test = data_test.values.tolist()
    data_array_test = np.array(data_array_test)
    data_array_test[[(np.where(data_array_test[:, 2] >0))], 2] = 1
    if ratio < 1.0:
        data_array_train = data_array_train[data_array_train[:, -1].argsort()[:int(ratio*len(data_array_train))]]
    # print('data_array_train', data_array_train.shape, data_array_test.shape)
    data_array = np.concatenate([data_array_train, data_array_test[:, :-1]], axis=0)
    # print('SHAPE!', data_array.shape, data_array_train.shape, data_array_test[:, :-1].shape)
    u_nodes_ratings = data_array[:, 0].astype(dtypes['u_nodes'])
    v_nodes_ratings = data_array[:, 1].astype(dtypes['v_nodes'])
    ratings = data_array[:, 2].astype(dtypes['ratings'])
    # print('!!!!!u_nodes_ratings', u_nodes_ratings)
    if rating_map is not None:
        for i, x in enumerate(ratings):
            ratings[i] = rating_map[x]

    u_nodes_ratings, u_dict, num_users = map_data(u_nodes_ratings)
    # print('u_nodes_ratings, u_dict)', u_nodes_ratings, u_dict)
    v_nodes_ratings, v_dict, num_items = map_data(v_nodes_ratings)
    # print('2!!!!!u_nodes_ratings', u_nodes_ratings)

    u_nodes_ratings, v_nodes_ratings = u_nodes_ratings.astype(np.int64), v_nodes_ratings.astype(np.int32)
    ratings = ratings.astype(np.int64)
    # print('ratings ', np.where(ratings==0))
    u_nodes = u_nodes_ratings
    v_nodes = v_nodes_ratings
    # print('u_nodes_ratings', u_nodes_ratings.shape)
    ...
    # print('pairs_nonzero', pairs_nonzero.shape)
    ...
    train_pairs_idx = pairs_nonzero[num_val:num_train + num_val]
    test_pairs_idx = pairs_nonzero[num_train + num_val:]
    # print('train_pairs_idx', train_pairs_idx)
    ...
    val_labels = labels[val_idx] 
    test_labels = labels[test_idx]
    print('train_labels', np.where(test_labels)[0].shape)
    ...
    # make training adjacency matrix
    rating_mx_train = np.zeros(num_users * num_items, dtype=np.float32)
    # print('post_rating_map', labels[train_idx])
    if post_rating_map is None:
        rating_mx_train[train_idx] = labels[train_idx].astype(np.float32) +1
        # print('rating_mx_train', rating_mx_train.reshape(num_users, num_items))
    else:
        rating_mx_train[train_idx] = np.array([post_rating_map[r] for r in class_values[labels[train_idx]]]) + 1.
    # print('rating_mx_train.shape,', rating_mx_train)
    rating_mx_train = sp.csr_matrix(rating_mx_train.reshape(num_users, num_items))
    ...
    # print("User features shape: "+str(u_features.shape))
    # print("Item features shape: "+str(v_features.shape))
    # print('rating_mx_test', max(test_labels))
    # print('rating_mx_train', max(train_labels))
    # print('u_train_idx', rating_mx_train.shape)
    u_features=None
    v_features=None
    return u_features, v_features, rating_mx_train, train_labels, u_train_idx, v_train_idx, \
        val_labels, u_val_idx, v_val_idx, test_labels, u_test_idx, v_test_idx, class_values
